...
with st.form("survey_form"):
    ...
    street = st.selectbox(
        ...
        ["중앙대로", "상남대로", "팔용산길", "반송로", "불종거리", "귀산카페거리", "그 외"]
    )
    cleanliness = st.slider(
        "해당 거리를 얼마나 꺠끗하다고 느끼나요?(1. 매우 불만족~5. 매우 만족)",
...
DATA_PATH = "data/reponses.csv"
os.makedirs("data", exist_ok = True)

if submitted:
    now = datetime.now().strftime("%Y-%m-%d%H:%M:%S")
    new_row = {"timestamp": now, "name": name, "street": street, "cleanliness": cleanliness, 
    "safety": safety, "comments": comments
    }
